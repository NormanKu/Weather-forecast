...
const Forecast = ({ data }: Props): JSX.Element => {
  ...
  return (
    <div className="w-full md:max-w- py-4 md:py-4 md:px-10 lg:px-24 h-full lg:h-auto bg-white bg-opacity-20 backdrop-blur-ls rounded drop-shadow-lg" >
      <div className="mx-auto w-[300px]">
        <section className="text-center">
          ...
          <h1 className="text-4xl font-extrabold">
            ...
            <p className="text-sm">
              H: <Degree temp={Math.round(today.main.temp_max)}/> L:
        ...
        <section className=" flex flex-wrap justify-between text-zinc-700">
          <div className="w-[140px] text-xs font-bold flex flex-col backdrop-blur-lg rounded drop-shadow-lg py-4 mb-5 items-center bg-white/20" >
            <Sunrise /> <span className="mt-2">{getSunTime(data.sunrise)}</span></div>
          <div className="w-[140px] text-xs font-bold flex flex-col backdrop-blur-lg rounded drop-shadow-lg py-4 mb-5 items-center bg-white/20" >
            <Sunset /> <span className="mt-2">{getSunTime(data.sunset)}</span></div>
{/* wind */}
...
{/* humidity */}
<Tile icon="humidity" title="Humidity" info={`${today.main.humidity} %`}  description={`${getWindDirection(Math.round(today.wind.deg))}, gusts ${today.wind.gust.toFixed(1)} km/h`}/>
